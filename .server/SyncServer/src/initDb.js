...
`);
  ...
  // Best-effort migration for v2 sync: add `rev` and initialize it.
  try {
    const [revRows] = await pool.query("SHOW COLUMNS FROM vault_files LIKE 'rev'");
    const hasRev = Array.isArray(revRows) && revRows.length > 0;
    if (!hasRev) {
      await pool.query("ALTER TABLE vault_files ADD COLUMN rev BIGINT UNSIGNED NOT NULL DEFAULT 0 AFTER path_hash");
    }
    await pool.query("UPDATE vault_files SET rev = 1 WHERE rev IS NULL OR rev <= 0").catch(() => undefined);
  } catch (_) {
    // ignore
  }
}
